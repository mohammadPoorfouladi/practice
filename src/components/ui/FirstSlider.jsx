...
const FirstSlider=()=>{
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex(prevIndex => {
        if (prevIndex +5> firstSliders.length) {
          return 0;
        }
        return prevIndex + 1;
      });
  ...
  const handleNext = () => {
    setCurrentIndex(prevIndex => {
      if (prevIndex - 1 >= firstSliders.length) {
        return 0; 
      }
      return prevIndex - 1;
    });
 ...
 <Box sx={{  mt: {xs:3.6,md:5}, overflow: 'hidden', width: "90.3%"}}>
      <Box
        sx={{
          ...
          width: { xs: `${(firstSliders.length / 1) * 100}%`,
           sm: `${(firstSliders.length / 2) * 100}%`,
           md: `${(firstSliders.length / 3) * 100}%`,
           lg: `${(firstSliders.length / 4) * 100}%`
        }
        }}
      >
