...
const NewClipSlider= () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const response = await axios.get(" https://api.masaf.ir/api/v1/portal/contentByPortal/?portalId=13&count=latest")
    ...
    <>
      <Box sx={{ overflow: 'hidden', width: '100%' }}>
        <Box sx={{
          ...
          justifyContent: "center", mt: { xs: 7, md: 6.2 },
          mb: 1.9
        }}>
            ...
            >
              {data.map((slider) => (
                <Card
                  ...
                  />
                  <CardContent sx={{ px: 1.3, height: 178.5, display: "flex", flexDirection: "column", justifyContent: "space-between", overflow: "hidden", mb: -3 }}>
                    <Box>
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.4 }}>
                      ...
                      <Box sx={{ m: "auto", display: "flex", alignItems: "center", gap: .2 }}><Button
                        sx={{
                          width: 35,
                          height: 18,
                          padding: 0,
                          boxShadow: 0,
                          border: .1, borderColor: grey[400], borderRadius: 0.3,
                          minWidth: 'auto',
                          '&:hover': {
                            boxShadow: 0,
                            backgroundColor: "white"
                          },
                          color: grey[800], fontSize: "12px"
                        }}
                        variant="outlined"
                      ...
                      >
                        ...
                        <Typography sx={{ fontSize: "11px" }} variant="caption">{slider.portal.name}</Typography>
                      </Box>
